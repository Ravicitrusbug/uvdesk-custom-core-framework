...
class UserRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function getAllAgents(ParameterBag $params = null, ContainerInterface $container, $group = '', $organization = '', $company = '')
    {
        ...
        $queryBuilder = $this->getEntityManager()->createQueryBuilder()
            ->select("user, userInstance, supportRole, supportCompany, supportGroup, supportTeam")
        ...
        foreach ($params as $field => $fieldValue) {
            ...
            if (!in_array($field, ['dateUpdated', 'dateAdded', 'search', 'isActive'])) {
                ...
                if ('search' == $field) {
                    $queryBuilder->andwhere("user.firstName LIKE :fullName OR user.email LIKE :email OR supportCompany.name LIKE :email OR supportGroup.name LIKE :email OR supportTeam.name LIKE :email")
        ...
        return [
            ...
            'users' => array_map(function ($user) {
                return [
                    ...
                    'companies' => implode(' , ', array_column($user['userInstance'][0]['supportCompanies'], 'name')),
                    'groups' => implode(' , ', array_column($user['userInstance'][0]['supportGroups'], 'name')),
                    'teams' => implode(' , ', array_column($user['userInstance'][0]['supportTeams'], 'name')),
    ...
    public function getUserSupportCompanyReferences(User $user)
    {
        $query = $this->getEntityManager()->createQueryBuilder()
            ->select('uc.id')->from('UVDeskCoreFrameworkBundle:User', 'u')
            ->leftJoin('u.userInstance', 'userInstance')
            ->leftJoin('userInstance.supportCompanies', 'uc')
            ->andwhere('u.id = :userId')
            ->setParameter('userId', $user->getId())
